...
class UserEditScreen extends Screen
{
     ...
     * @return array
     */
    private ?\App\Services\AdvisualRequisitionService $advisual = null;

    private function advisual(): \App\Services\AdvisualRequisitionService
    {
        return $this->advisual ??= app(\App\Services\AdvisualRequisitionService::class);
    }
    ...
    public function query(User $user): iterable
    {
        ...
        // Sugerir el solicitante Advisual por email cuando aún no hay uno asignado.
        if (empty($user->advisual_usuario_guid)) {
            $user->advisual_usuario_guid = $this->advisual()->suggestGuidForEmail($user->email);
        }
    ...
    public function layout(): iterable
    {
        return [
            ...
            Layout::block(Layout::rows([
                Select::make('user.advisual_usuario_guid')
                    ->options($this->advisual()->listUsuarios())
                    ->empty('— Sin asignar —')
                    ->title('Usuario Advisual (solicitante)')
                    ->help('Se sugiere automáticamente por email si coincide; verifica antes de guardar.'),
            ]))
                ->title('Advisual')
                ->description('Vincula este usuario con su solicitante en Advisual.'),
